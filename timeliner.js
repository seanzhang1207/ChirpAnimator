...
function Timeliner(target, onTimeUpdate, onDeleteKeyframe, onLoadJSONFile, onNewKeyframe, onSaveFrame, onLoadFrame) {
	...
	dispatcher.on('keyframe', function(layer, value) {
		...
		if (typeof(v) === 'number') {
			...
			layer.values.splice(v, 0, key);

			if (onNewKeyframe) {
				key = onNewKeyframe(index, key);
			}
	...
	dispatcher.on('controls.toggle_play', function() {
		if (start_play) {
			//console.log("pause")
			//console.log(data.get('ui:currentTime').value)
			pausePlaying();
		} else {
			//console.log("start")
			//console.log(data.get('ui:currentTime').value)
	...
	*/
	...
	function saveSimply() {
		...
		*/
		...
		if (onSaveFrame) {
			var ret = prompt('Filename', 'Frame');

			//console.log(JSON.stringify(data.data, null, '\t'));
			if (!ret) return;

			// make json downloadable
			var json = onSaveFrame();
			var fileName = ret + '.json';
	...
	function exportJSON() {
		var json = data.getJSONString();
		var ret = prompt('Filename', 'Animation');
	...
	*/
	...
	this.getLayer = function (id) {
		return layer_store.value[id];
	};
	this.getLayers = function (id) {
...
/*
	// save as
	var save_as = new IconButton(16, 'cog', 'Save as', dispatcher);
